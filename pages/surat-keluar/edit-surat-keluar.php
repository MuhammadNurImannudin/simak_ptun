...
// Get existing data
$surat = $db->fetch("SELECT * FROM surat_keluar WHERE id = ?", [$id]);
if (!$surat) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!isset($errors['nomor_surat'])) {
        $existing = $db->fetch("SELECT id FROM surat_keluar WHERE nomor_surat = ? AND id != ?", [$nomor_surat, $id]);
        if ($existing) {
    ...
    if (isset($_FILES['file_surat']) && $_FILES['file_surat']['error'] === UPLOAD_ERR_OK) {
        try {
            $new_file_path = uploadFile($_FILES['file_surat'], 'surat-keluar');
    ...
    if (empty($errors)) {
        try {
            ...
            $db->query($notification_sql, [
                $_SESSION['user_id'],
                'Surat Keluar Diperbarui',
                "Surat keluar {$nomor_surat} berhasil diperbarui",
                'info'
            ]);
            
            setFlashMessage('success', 'Surat keluar berhasil diperbarui');
...
<main class="main-content">
        ...
        <div class="d-flex justify-content-between align-items-center">
            <div>
                ...
                <p class="page-subtitle">
                    Edit data surat keluar: <?= htmlspecialchars($surat['nomor_surat']) ?>
                </p>
            </div>
    ...
    <div class="card">
        <div class="card-header">
            <h5 class="card-title">Form Edit Surat Keluar</h5>
        </div>
        <div class="card-body">
                ...
                <div class="form-group">
                    ...
                    <div class="d-flex gap-2 flex-wrap">
                        <?php if ($surat['status'] === 'draft'): ?>
                            <button type="button" class="btn btn-success btn-sm" onclick="saveAndSend()">
                                <i class="fas fa-paper-plane"></i>
                                Simpan & Kirim
                            </button>
                        <?php endif; ?>
                        <button type="button" class="btn btn-info btn-sm" onclick="saveAsTemplate()">
...
function saveAsTemplate() {
    ...
    if (tujuan && perihal) {
        ...
        let templates = JSON.parse(localStorage.getItem("suratKeluarTemplates") || "[]");
        ...
        localStorage.setItem("suratKeluarTemplates", JSON.stringify(templates));
        showSuccessToast("Template berhasil disimpan");
    } else {
        showWarningToast("Silakan isi tujuan dan perihal untuk menyimpan template");
    }
}
